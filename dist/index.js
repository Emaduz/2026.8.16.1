...
async function saveSection(input) {
  const db = requireDb(await getDb());
  await db.insert(siteSections).values({ ...input }).onDuplicateKeyUpdate({
    set: {
...
var ownerProcedure = adminProcedure.use(({ ctx, next }) => {
  if (getCookieValue(ctx.req, ADMIN_PASSWORD_COOKIE) !== getAdminAccessToken()) {
    throw new TRPCError3({ code: "UNAUTHORIZED", message: "Admin password required" });
  }
  if (!ENV.ownerOpenId || ctx.user.openId !== ENV.ownerOpenId) {
    throw new TRPCError3({ code: "FORBIDDEN", message: "Owner access only" });
  }
  return next({ ctx });
});
...
var appRouter = router({
  ...
  content: router({
    ...
    updateProject: ownerProcedure.input(projectSchema.extend({ id: z2.number().int().positive() })).mutation(async ({ input }) => {
      ...
      await db.update(projects).set({ ...values, published: values.published ? 1 : 0, updatedAt: /* @__PURE__ */ new Date() }).where(eq2(projects.id, id));
      if (slides !== void 0) {
    ...
    updatePost: ownerProcedure.input(postSchema.extend({ id: z2.number().int().positive() })).mutation(async ({ input }) => {
      ...
      await db.update(posts).set({ ...values, published: values.published ? 1 : 0, updatedAt: /* @__PURE__ */ new Date() }).where(eq2(posts.id, id));
